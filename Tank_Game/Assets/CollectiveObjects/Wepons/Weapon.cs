...
public class Weapon : MonoBehaviour {
	...
	private void Update () {
		...
		if (Mathf.Abs(basic.moveSpeed)==Mathf.Infinity) {		// Is it a laser?
			RaycastHit hit;
		...
		translate = transform.InverseTransformPoint(transform.position+transform.forward);		// All weapons, if they move, only move forward.
		if (fuel>0) {
			fuel -= Time.deltaTime;
...
//		other.health-damage;		// Do Damage.			// Tells it to update the TextMesh.
		Destroy(gameObject);
...
//			//Instantiate(prettyParticleExplodSphere);
			...
			for (int j=0; j<colliders.Length; j++) {
				...
				if (found) {
					found.healthCurrent -= Mathf.Clamp01(1-Vector3.Distance(explodeOrigin, impact.transform.position)/damageRange)*damageMax;
				}
			}
		}
		Destroy(gameObject);
